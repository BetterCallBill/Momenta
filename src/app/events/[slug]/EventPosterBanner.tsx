  ...
  if (isPortrait === true) {
    return (
      <div className="w-full bg-brand-black">
        <div className="mx-auto w-full max-w-md px-6 pt-28 pb-14 md:px-0">
          <BackLink />

          {/* Poster image — natural aspect ratio, no cropping */}
          <img
            src={coverImageUrl!}
            alt={title}
            className="mt-6 w-full h-auto rounded-2xl shadow-2xl shadow-black/70"
          />

          {/* Event identity below poster */}
          <div className="mt-6 space-y-3">
            <SportBadge sportType={sportType} isFeatured={isFeatured} />

            <h1 className="text-2xl font-extrabold text-white leading-tight md:text-3xl">
              {title}
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-white/60">
              <span className="flex items-center gap-1.5">
                ...
                {formatDateLong(new Date(startAt))}{" · "}
                {formatTime(new Date(startAt))} – {formatTime(new Date(endAt))}
              </span>
              <span className="flex items-center gap-1.5">
                <span aria-hidden="true">📍</span>
                {locationName}
